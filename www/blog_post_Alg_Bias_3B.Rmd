...
```{r img_blog_post_Alg_Bias_3B_Achilles_being_hit_in_the_heel-black-and-white, fig.cap="*Source: Twinkl.com*", echo=FALSE, out.width="100%"}
...
Let:
non-matching NN = the number of nearest neighbor points that do not match the input’s final output label
total = the total number of nearest neighbors considered
Credibility score = $\frac{non-matching NN}{total}$
...
In mathmatical notation, the confidence score can be written as:

[ADD IN THE MATH NOTATION YOU WROTE ON THE WHITEBOARD]
...
Please note that this approach works for deep learning that pushes each input through a the same series of layers. The confidence and credibility score approach would not work in a neural network model where each input is analyzed by some, but not all, of the model's neurons.
